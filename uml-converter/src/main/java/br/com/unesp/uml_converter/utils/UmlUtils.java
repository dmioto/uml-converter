...
public class UmlUtils {
    ...
    private static String attributeToUml(List<BaseAttr> attributes) {
        ...
        for (BaseAttr attr : attributes) {
            attrsUML.append(String.format("    %s %s %s", verifyModifier(attr.getModifier()), attr.getType(), attr.getName()));
    ...
    private static String relationshipToUML(List<BaseRelationship> relationships) {
        ...
        for (BaseRelationship attr : relationships) {
            relationUML.append(String.format("%s %s %s", attr.getObject1(), verifyRelantionshipMultiplicity(attr) , attr.getObject2()));
        }
    ...
    private static String verifyRelantionshipMultiplicity(BaseRelationship baseRM) {
        ...
        String strRM = String.format("%s %s %s", baseRM.getObject1(), verifyRelation(baseRM.getRelation()), baseRM.getObject2());
